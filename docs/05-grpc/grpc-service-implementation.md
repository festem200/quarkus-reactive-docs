...
> **Guia oficial:** <https://quarkus.io/guides/grpc-service-implementation>  
> **Fuente:** `docs/src/main/asciidoc/grpc-service-implementation.adoc` en [quarkusio/quarkus@3.38.2](https://github.com/quarkusio/quarkus/blob/3.38.2/docs/src/main/asciidoc/grpc-service-implementation.adoc)  
...
**❗ IMPORTANT**\
You’ll need to implement the _service interface_ or extend the _base class_ with your service implementation bean as described in the following sections.
...
[gRPC documentation](https://github.com/grpc/grpc/blob/v1.28.1/doc/health-checking.md)
...
Quarkus gRPC Server implements the [reflection service](https://github.com/grpc/grpc/blob/master/doc/server-reflection.md).
This service allows tools like [grpcurl](https://github.com/fullstorydev/grpcurl) or [grpcox](https://github.com/gusaul/grpcox) to interact with your services.
...
**📌 NOTE**\
La tabla de configuracion generada `quarkus-grpc_quarkus.grpc.server` se produce al construir la documentacion y no existe en el codigo fuente. Consulta la referencia de configuracion en https://quarkus.io/guides/all-config
...
**💡 TIP**\
Check the [ServerInterceptor JavaDoc](https://grpc.github.io/grpc-java/javadoc/io/grpc/ServerInterceptor.html) to properly implement your interceptor.
...
in [Consuming a gRPC Service](grpc-service-consumption.md).
...
gRPC server metrics are automatically enabled when the application also uses the [`quarkus-micrometer`](../07-observabilidad/telemetry-micrometer.md) extension.
...
To use virtual threads in your gRPC service implementation, check the dedicated [guide](grpc-virtual-threads.md).

## gRPC Server authorization

Quarkus includes built-in security to allow [authorization using annotations](https://quarkus.io/guides/security-authorize-web-endpoints-reference#standard-security-annotations) when the Vert.x gRPC support, which uses existing Vert.x HTTP server, is enabled.
...
|Username and password |[Basic authentication](#basic-authentication)

|Client certificate |[Mutual TLS authentication](#mutual-tls-authentication)

|Custom requirements |[Custom authentication](#custom-authentication)

|Bearer access token |[OIDC Bearer token authentication](https://quarkus.io/guides/security-oidc-bearer-token-authentication), [JWT](https://quarkus.io/guides/security-jwt), [OAuth2](https://quarkus.io/guides/security-oauth2)
...
Please refer to the [Basic authentication guide](https://quarkus.io/guides/security-basic-authentication-howto) for example how to provide the `IdentityProvider` based on username and password.

**💡 TIP**\
If you use separate HTTP server to serve gRPC requests, [Custom authentication](#custom-authentication) is your only option.
Set the `quarkus.grpc.server.use-separate-server` configuration property to `false` so that you can use other mechanisms.

### Secure gRPC service

The gRPC services can be secured with the [standard security annotations](https://quarkus.io/guides/security-authorize-web-endpoints-reference#standard-security-annotations) like in the example below:
...
Most of the examples of the supported mechanisms sends authentication headers, please refer to the [gRPC Headers](grpc-service-consumption.md)
section of the Consuming a gRPC Service guide for more information about the gRPC headers.

### Basic authentication

Quarkus Security provides built-in authentication support for the [Basic authentication](https://quarkus.io/guides/security-basic-authentication).
...
The simplest way to enforce authentication for all your gRPC services is described in the [TLS with Mutual Auth](#tls-with-mutual-auth) section of this guide.
