...
> **Guia oficial:** <https://quarkus.io/guides/redis-reference>  
> **Fuente:** `docs/src/main/asciidoc/redis-reference.adoc` en [quarkusio/quarkus@3.38.2](https://github.com/quarkusio/quarkus/blob/3.38.2/docs/src/main/asciidoc/redis-reference.adoc)  
...
1. The [interactive tutorial](https://try.redis.io/) introducing Redis.
2. The [command references](https://redis.io/commands/) explains Redis commands and contains links to reference documentation.
...
More details about the various APIs offered by the quarkus-redis extension are available in the [One extension, multiple APIs](#one-extension-multiple-apis) section.

<dl><dt><strong>📌 NOTE</strong></dt><dd>

To use Redis as a cache backend, refer to the [Redis Cache Backend reference](cache-redis-reference.md).
</dd></dl>
...
More details on [the Vert.x Redis Client documentation](https://vertx.io/docs/vertx-redis-client/java/).
...
## <a name="inject-the-default-and-named-clients"></a> Inject the default and named clients
...
* The Redis Client does not contribute a [health check](#enable-the-health-checks).
...
Setting `quarkus.redis.one.active=true` [at runtime](../01-fundamentos/config-reference.md#configuration-sources) makes only the
...
1. Set the `quarkus.redis.tls.enabled=true` property or use the  [TLS registry](../08-rendimiento-nativo/tls-registry-reference.md) (recommended)
...
This API is type-safe and structured around the notion of _group_, inherited from the [Redis command organization](https://redis.io/commands/command-docs/).
...
<dl><dt><strong>📌 NOTE: All about delegation</strong></dt><dd>
...
* json - `.json()` (requires the [RedisJSON](https://redis.io/docs/latest/develop/data-types/json/) module on the server side)
* bloom - `.bloom()` (requires the [RedisBloom](https://redis.io/docs/latest/develop/data-types/probabilistic/bloom-filter/) module on the server side)
* cuckoo - `.cuckoo()` (requires the [RedisBloom](https://redis.io/docs/latest/develop/data-types/probabilistic/bloom-filter/) module on the server side, which also provides the cuckoo filter commands)
* count-min - `.countmin()` (requires the [RedisBloom](https://redis.io/docs/latest/develop/data-types/probabilistic/bloom-filter/) module on the server side, which also provides the count-min filter commands)
* top-k - `.topk()` (requires the [RedisBloom](https://redis.io/docs/latest/develop/data-types/probabilistic/bloom-filter/) module on the server side, which also provides the top-k filter commands)
* graph - `.graph()` (requires the [RedisGraph](https://redis.io/docs/latest/operate/oss_and_stack/stack-with-enterprise/deprecated-features/graph/) module on the server side).
These commands are marked as experimental.
Also the module has been declared _end of life_ by [Redis](https://redis.io/blog/redisgraph-eol/).
* search - `.search()` (requires the [RedisSearch](https://redis.io/docs/latest/develop/interact/search-and-query/) module on the server side).
* auto-suggest - `.autosuggest()` (requires the [RedisSearch](https://redis.io/docs/latest/develop/interact/search-and-query/) module on the server side).
* time-series - `.timeseries()` (requires the [Redis Time Series](https://redis.io/docs/latest/develop/data-types/timeseries/) module on the server side).
...
The `value` group is used to manipulate [Redis Strings](https://redis.io/docs/manual/data-types/#strings).
...
The documentation of the Vert.x Redis Client is available on the [Vert.x Web Site](https://vertx.io/docs/vertx-redis-client/java/).
...
See [Redis Dev Service](redis-dev-services.md).
...
Redis client metrics are automatically enabled when the application also uses the [`quarkus-micrometer`](../07-observabilidad/telemetry-micrometer.md) extension.
...
**📌 NOTE**\
La tabla de configuracion generada `quarkus-redis-client` se produce al construir la documentacion y no existe en el codigo fuente. Consulta la referencia de configuracion en https://quarkus.io/guides/all-config
