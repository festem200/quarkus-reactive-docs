...
> **Guia oficial:** <https://quarkus.io/guides/flyway>  
> **Fuente:** `docs/src/main/asciidoc/flyway.adoc` en [quarkusio/quarkus@3.38.2](https://github.com/quarkusio/quarkus/blob/3.38.2/docs/src/main/asciidoc/flyway.adoc)  
> **Version documentada:** Quarkus 3.38.2 · **Sincronizado:** 2026-08-17 · **Licencia:** Apache-2.0

[Flyway](https://flywaydb.org/) is a popular database migration tool that is commonly used in JVM environments.

Quarkus provides first class support for using Flyway as will be explained in this guide.

## Setting up support for Flyway

As shown in the [Developing with Flyway](#developing-with-flyway) section, to start using Flyway with your project, you just need to:
...
* unless you’re using in-memory or file databases (such as H2 or SQLite), you need to add a flyway module dependency corresponding to the database you’re using. ([for more details](https://github.com/flyway/flyway/issues/3780))
...
It can be customized for the default datasource as well as for every [named datasource](#multiple-datasources).
First, you need to add the datasource config to the `application.properties` file
...
**📌 NOTE**\
La tabla de configuracion generada `quarkus-flyway` se produce al construir la documentacion y no existe en el codigo fuente. Consulta la referencia de configuracion en https://quarkus.io/guides/all-config
...
[application startup](../01-fundamentos/lifecycle.md).
...
In such situations the [Flyway repair command](https://flywaydb.org/documentation/command/repair) comes in handy. In Quarkus this can either be executed automatically before the migration by setting `quarkus.flyway.repair-at-start=true` or manually by injecting the `Flyway` object and calling `Flyway#repair()`.
...
You can find more information about this feature in the [Hibernate ORM guide](https://quarkus.io/guides/hibernate-orm#flyway).

## Flyway and Reactive datasources

Flyway internally relies on a JDBC datasource,
whereas reactive use cases will rely on [reactive SQL clients](reactive-sql-clients.md),
either directly or through [Hibernate Reactive](hibernate-reactive.md).
This is not a problem in Quarkus,
because [a single configured datasource can be made available both through reactive clients and JDBC](datasource.md#jdbc-and-reactive-datasources-simultaneously).

To use Flyway on a datasource you otherwise access reactively,
simply make sure to configure that datasource
both as [JDBC](datasource.md#jdbc-datasource)
and [reactive](datasource.md#reactive-datasource).
