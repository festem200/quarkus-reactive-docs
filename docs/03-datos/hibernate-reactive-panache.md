...
> **Guia oficial:** <https://quarkus.io/guides/hibernate-reactive-panache>  
> **Fuente:** `docs/src/main/asciidoc/hibernate-reactive-panache.adoc` en [quarkusio/quarkus@3.38.2](https://github.com/quarkusio/quarkus/blob/3.38.2/docs/src/main/asciidoc/hibernate-reactive-panache.adoc)  
> **Version documentada:** Quarkus 3.38.2 · **Sincronizado:** 2026-08-17 · **Licencia:** Apache-2.0

[Hibernate Reactive](https://hibernate.org/reactive/) is the only reactive Jakarta Persistence (formerly known as JPA) implementation and offers you the full
...
Hibernate Reactive is not a replacement for [Hibernate ORM](https://quarkus.io/guides/hibernate-orm-panache) or the future of Hibernate ORM.
...
Hibernate with Panache also allows for the use of the more classical [repository pattern](https://martinfowler.com/eaaCatalog/repository.html) via `PanacheRepository`.
...
The solution is located in the `hibernate-reactive-panache-quickstart` [directory](https://github.com/quarkusio/quarkus-quickstarts/tree/main/hibernate-reactive-panache-quickstart).
...
Follow the [Hibernate set-up guide for all configuration](https://quarkus.io/guides/hibernate-orm#setting-up-and-configuring-hibernate-orm).
...
2. Configure Hibernate ORM to create the schema on startup in production, which is useful for experimentation, but rely on [convenient defaults in tests / dev mode](https://quarkus.io/guides/hibernate-orm#dev-mode).
...
[HQL](https://docs.hibernate.org/orm/{hibernate-orm-version-major-minor}/userguide/html_single/#hql):
...
[hql select clause](https://docs.hibernate.org/orm/{hibernate-orm-version-major-minor}/userguide/html_single/#hql-select-clause)
...
public class RaceWeight {
    ...
    public RaceWeight(String race, Double weight) { // ①
        this.race = race;
        this.weight = weight;
    }
}
...
For configuration details on setting up multiple persistence units, refer to the [Multiple Persistence Units](https://quarkus.io/guides/hibernate-orm#multiple-persistence-units) section in the Hibernate ORM guide.
...
**📌 NOTE**\
Note that a Panache entity may not be used from a blocking thread. See also [Getting Started With Reactive](../01-fundamentos/getting-started-reactive.md) guide that explains the basics of reactive principles in Quarkus.
...
See the [Hibernate Reactive guide](hibernate-reactive.md) for more details on using `@Transactional` with Hibernate Reactive.
</dd></dl>
<dl><dt><strong>⚠️ WARNING</strong></dt><dd>

There are some limitations when using different pipelines with `@Transactional`, refer to [the limitations section in the Hibernate Reactive guide](hibernate-reactive.md#transactional-different-pipelines)
</dd></dl>
...
The usage is described in the [Hibernate Reactive](hibernate-reactive.md#testing) guide.
...
public class PanacheFunctionalityTest {
    ...
    @RunOnVertxContext // ①
    @Test
    public void testPanacheRepositoryMocking(UniAsserter asserter) { // ②
