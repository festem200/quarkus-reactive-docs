...
> **Guia oficial:** <https://quarkus.io/guides/mongodb>  
> **Fuente:** `docs/src/main/asciidoc/mongodb.adoc` en [quarkusio/quarkus@3.38.2](https://github.com/quarkusio/quarkus/blob/3.38.2/docs/src/main/asciidoc/mongodb.adoc)  
...
The solution is located in the `mongodb-quickstart` [directory](https://github.com/quarkusio/quarkus-quickstarts/tree/main/mongodb-quickstart).
...
See [MongoDB Dev Services](mongodb-dev-services.md).
...
* Do not set `quarkus.mongodb[.optional name].hosts` or `quarkus.mongodb[.optional name].connection-<mark>string</mark>`.
...
* The Mongo Client does not contribute a [health check](#connection-health-check).
...
Setting `quarkus.mongodb.one.active=true` [at runtime](../01-fundamentos/config-reference.md#configuration-sources) makes only the
...
If you use [Dev Services](#use-the-mongodb-dev-services), launching the container manually is not necessary.
</dd></dl>
...
In the `src/main/resources/META-INF/resources` directory, add a `fruits.html` file with the content from this [fruits.html](https://github.com/quarkusio/quarkus-quickstarts/blob/main/mongodb-quickstart/src/main/resources/META-INF/resources/fruits.html) file in it.
...
The [POJO Codec](https://www.mongodb.com/docs/drivers/java/sync/current/fundamentals/data-formats/document-data-format-pojo/) provides a set of annotations that enable the customization of
...
The [MongoDB with Panache](mongodb-panache.md) extension facilitates the usage of MongoDB by providing active record style entities (and repositories) like you have in [Hibernate ORM with Panache](https://quarkus.io/guides/hibernate-orm-panache) and focuses on making your entities trivial and fun to write in Quarkus.

## Schema migration with Liquibase

The [Liquibase MongoDB](https://quarkus.io/guides/liquibase-mongodb) extension facilitates the initialization of a MongoDB database including indices and initial data.
...
To use tracing with MongoDB, you need to add the [`quarkus-opentelemetry`](../07-observabilidad/opentelemetry.md) extension to your project.
...
[Dev Services for MongoDB](#use-the-mongodb-dev-services) is your best option to start a MongoDB database for your unit tests.
...
They rely on [Flapdoodle embedded MongoDB](https://github.com/flapdoodle-oss/de.flapdoodle.embed.mongo).
...
For more information about the usage of a `QuarkusTestResourceLifecycleManager` please read [Quarkus test resource](../09-testing/getting-started-testing.md#quarkus-test-resource).
...
Currently, Quarkus doesn’t support [Client-Side Field Level Encryption](https://docs.mongodb.com/manual/core/security-client-side-encryption/) in native mode.
</dd></dl>
...
Follows the instructions from [the Mongo web site](https://www.mongodb.com/docs/manual/core/csfle/) to configure CSFLE:
...
**❗ IMPORTANT**\
Client-side field level encryption, and feature relying on [Mongo Crypt](https://github.com/mongodb/libmongocrypt) in general, are not supported in native mode.

## Configuration Reference

**📌 NOTE**\
La tabla de configuracion generada `quarkus-mongodb-client` se produce al construir la documentacion y no existe en el codigo fuente. Consulta la referencia de configuracion en https://quarkus.io/guides/all-config
