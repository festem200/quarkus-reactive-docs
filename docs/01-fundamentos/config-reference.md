...
> **Guia oficial:** <https://quarkus.io/guides/config-reference>  
> **Fuente:** `docs/src/main/asciidoc/config-reference.adoc` en [quarkusio/quarkus@3.38.2](https://github.com/quarkusio/quarkus/blob/3.38.2/docs/src/main/asciidoc/config-reference.adoc)  
...
the [SmallRye Config](https://github.com/smallrye/smallrye-config) API an implementation of the
...
[Writing Your Own Extension](https://quarkus.io/guides/writing-extensions#configuration) guide.
...
3. (295) [.env](#env-file-in-the-current-working-directory) file in the current working directory
4. (260) [Quarkus Application configuration file](#quarkus-application-configuration-file) in `$PWD/config/application.properties`
5. (250) [Quarkus Application configuration file](#quarkus-application-configuration-file) `application.properties` in classpath
6. (100) [MicroProfile Config configuration file](#microprofile-config-configuration-file)
...
![config-sources](../_assets/config-sources.png)
...
```
1. The name `QUARKUS_DATASOURCE_PASSWORD` follows the same conversion rules used for [Environment variables](#environment-variables).
...
* [YAML](../10-extras/config-yaml.md)
* [HashiCorp Vault](https://github.com/quarkiverse/quarkus-vault)
...
**💡 TIP**\
It is also possible to create a [Custom Config Source](https://quarkus.io/guides/config-extending-support#custom-config-source).

## ConfigMapping

The recommended way to expose configuration to a Quarkus application is by mapping it with an interface. With
[SmallRye Config Mappings](https://smallrye.io/smallrye-config/Main/config/mappings/), it is possible to
...
**💡 TIP**\
Check the [Mapping configuration to objects](../10-extras/config-mappings.md) guide for advanced usages with
`@ConfigMapping`.

## ConfigProperty

While `@ConfigMapping` is the recommended approach, Quarkus also supports
[MicroProfile Config](https://microprofile.io/project/eclipse/microprofile-config) annotations to inject the
...
**💡 TIP**\
Use [Config Mappings](../10-extras/config-mappings.md#config-mappings) to group similar configuration properties.
...
Use [expressions](#property-expressions) to access it: `${quarkus.uuid}`.
...
`application.properties`. All the Quarkus configuration properties are [documented and searchable](../10-extras/all-config.md).
...
**💡 TIP**\
The properties fixed at build time are marked with a lock icon (icon:lock[]) in the [list of all configuration options](../10-extras/all-config.md).
...
then check out how [re-augmentation](https://quarkus.io/guides/reaugmentation) can be used to rebuild the augmentation output for a
...
The reason `${project.basedir}/.quarkus` directory was chosen as the default location was to make it easy to track build time configuration changes between builds and use that as an indicator to build output caching tools (such as [Apache Maven Build Cache](https://maven.apache.org/extensions/maven-build-cache-extension/) and [Develocity Build Cache](https://gradle.com/gradle-enterprise-solutions/build-cache/)) whether the application binary has to be re-built.
...
Quarkus collects the config property values injected in CDI beans during [static initialization phase](https://quarkus.io/guides/writing-extensions#bootstrap-three-phases).
...
* [YAML ConfigSource Extension](../10-extras/config-yaml.md)
...
* [Mapping configuration to objects](../10-extras/config-mappings.md)
...
**📌 NOTE**\
La tabla de configuracion generada `quarkus-core_quarkus.config` se produce al construir la documentacion y no existe en el codigo fuente. Consulta la referencia de configuracion en https://quarkus.io/guides/all-config
