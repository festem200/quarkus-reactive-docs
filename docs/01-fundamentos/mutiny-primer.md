...
> **Guia oficial:** <https://quarkus.io/guides/mutiny-primer>  
> **Fuente:** `docs/src/main/asciidoc/mutiny-primer.adoc` en [quarkusio/quarkus@3.38.2](https://github.com/quarkusio/quarkus/blob/3.38.2/docs/src/main/asciidoc/mutiny-primer.adoc)  
> **Version documentada:** Quarkus 3.38.2 · **Sincronizado:** 2026-08-17 · **Licencia:** Apache-2.0

[Mutiny](https://smallrye.io/smallrye-mutiny) is an intuitive, reactive programming library.
...
If you want to know more about Mutiny, and the concepts behind it, check [the Mutiny Reference documentation](https://smallrye.io/smallrye-mutiny/latest/reference/why-is-asynchronous-important).
...
***Non-Blocking I/O*** - Mutiny is the perfect companion to tame the asynchronous nature of applications with non-blocking I/O (which powers [Quarkus](quarkus-reactive-architecture.md)).
...
***Reactive Streams and Converters Built-In*** - Mutiny is based on the [Reactive Streams](https://www.reactive-streams.org/) specification, and so it can be integrated with any other reactive programming library.
In addition, it proposes converters to interact with other popular libraries.

## Mutiny and the I/O Threads

Quarkus is powered by a [reactive engine](quarkus-reactive-architecture.md#engine), and when developing a reactive application, your code is executed on one of the few I/O threads.
...
![reactive-thread](../_assets/reactive-thread.png)
...
Let’s imagine a simple structure representing an element from the [periodic table](https://en.wikipedia.org/wiki/Periodic_table):
...
More details about transformation can be found in the [Mutiny documentation](https://smallrye.io/smallrye-mutiny/latest/tutorials/transforming-items/).
...
More details about asynchronous transformation can be found in the [Mutiny documentation](https://smallrye.io/smallrye-mutiny/latest/tutorials/transforming-items-asynchronously/).
...
More info about failure recovery can be found on [the handling failure documentation](https://smallrye.io/smallrye-mutiny/latest/tutorials/handling-failures/) and [the retrying on failures documentation](https://smallrye.io/smallrye-mutiny/latest/tutorials/retrying/).
...
| --- | --- | --- |
| Uni | Subscription (1), Item (0..1), failure (0..1) | Cancellation |
| Multi | Subscription (1), Item (0..n), failure (0..1), completion (0..1) | Cancellation, Request |

Check the full list of events on [the event documentation](https://smallrye.io/smallrye-mutiny/latest/tutorials/observing-events/).

| Action | API | Comment |
| --- | --- | --- |
| transform | `onItem().transform(Function<I, O> function);` | Transform the event into another event using a synchronous function. The downstream receives the result of the function (or a failure if the transformation failed). |
...
Head over to the [Mutiny documentation](https://smallrye.io/smallrye-mutiny) to see many more examples, including the whole list of events and how to handle them.
...
Mutiny uses [Reactive Streams](https://www.reactive-streams.org/).
