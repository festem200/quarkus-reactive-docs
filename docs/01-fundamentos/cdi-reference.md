...
> **Guia oficial:** <https://quarkus.io/guides/cdi-reference>  
> **Fuente:** `docs/src/main/asciidoc/cdi-reference.adoc` en [quarkusio/quarkus@3.38.2](https://github.com/quarkusio/quarkus/blob/3.38.2/docs/src/main/asciidoc/cdi-reference.adoc)  
> **Version documentada:** Quarkus 3.38.2 · **Sincronizado:** 2026-08-17 · **Licencia:** Apache-2.0

Quarkus DI solution (also called ArC) is based on the [Jakarta Contexts and Dependency Injection 4.1](https://jakarta.ee/specifications/cdi/4.1/jakarta-cdi-spec-4.1.html) specification.
...
See also [the list of supported features and limitations](#a-namesupported_featuresalimitations-supported-features-and-limitations).
...
**💡 TIP**\
If you’re new to CDI we recommend you to read the [Introduction to CDI](cdi.md) first.
...
There is only single bean archive with the [bean discovery mode `annotated`](https://jakarta.ee/specifications/cdi/4.1/jakarta-cdi-spec-4.1.html#default_bean_discovery) and no visibility boundaries.
...
Bean classes that don’t have a [bean defining annotation](https://jakarta.ee/specifications/cdi/4.1/jakarta-cdi-spec-4.1.html#bean_defining_annotations) are not discovered.
...
```
You can find the latest plugin version in the [Gradle Plugin Portal](https://plugins.gradle.org/plugin/org.kordamp.gradle.jandex)
...
In Quarkus, we have [Qute](../02-web-http/qute-reference.md#injecting-beans-directly-in-templates).
...
One of the limitations of GraalVM is the usage of [Reflection](https://www.graalvm.org/jdk21/reference-manual/native-image/dynamic-features/Reflection/).
...
## <a name="supported_features"></a>[[limitations]] Supported Features and Limitations
...
* `@SessionScoped`
  * Only with the Undertow extension; see [here](cdi.md#bean-scope-available) for details
...
* Use the bean in an observer of the `StartupEvent` - normal scoped beans must be used as described in [Lazy By Default](#lazy-by-default):
...
* Annotate the bean with `@io.quarkus.runtime.Startup` as described in [Startup annotation](lifecycle.md#startup_annotation):
...
**📌 NOTE**\
Quarkus users are encouraged to always prefer the `@Observes StartupEvent` to `@Initialized(ApplicationScoped.class)` as explained in the [Application Initialization and Termination](lifecycle.md) guide.
...
Users and extension authors have several options [how to eliminate false positives](#how-to-eliminate-false-positives).
...
* is [excluded from removal](#how-to-eliminate-false-positives), or
...
**💡 TIP**\
`@IfBuildProperty` and `@UnlessBuildProperty` are repeatable annotations and may be put on a stereotype.
A bean will only be enabled if ***all*** the conditions defined by these annotations are satisfied.
...
**📌 NOTE**\
Properties set at runtime have absolutely no effect on the bean resolution using `@IfBuildProperty` / `@UnlessBuildProperty`.
...
**💡 TIP**\
`@LookupIfProperty` and `@LookupUnlessProperty` are repeatable annotations and may be put on a stereotype.
...
class MyProducer {
    @Produces
    MyClass produce(InterceptionProxy<MyClass> proxy) { // ①
...
The intercepted class should be [proxyable](https://jakarta.ee/specifications/cdi/4.1/jakarta-cdi-spec-4.1#unproxyable) and therefore should not be `final`, should not have non-private `final` methods, and should have a non-private zero-parameter constructor.
If it isn’t, a bytecode transformation will attempt to fix it if [enabled](#ability-to-handle-final-classes-and-methods), but note that adding a zero-parameter constructor is not always possible.
...
class MyProducer {
    @Produces
    MyClass produce(@BindingsSource(MyClassBindings.class) InterceptionProxy<MyClass> proxy) { // ②
        return proxy.create(new MyClass());
    }
}
```
...
[Interception of Static Methods](#interception-of-static-methods) is not supported for producer methods and synthetic beans.
...
public MyClass create(SyntheticCreationalContext<MyClass> context) {
    InterceptionProxy<MyClass> proxy = context.getInterceptionProxy(); // ①
    return proxy.create(new MyClass());
}
```
...
public void register(RegistrationContext context) {
    context.configure(MyClass.class)
            .types(MyClass.class)
            .injectInterceptionProxy(MyClassBindings.class) // ①
            .creator(MyClassCreator.class)
            .done();
}
```
...
In ArC, this is only true in the [Strict Mode](#strict-mode).
...
As documented in [the `@Startup` method documentation](lifecycle.md#startup_annotation), you may define startup
...
Nevertheless, most of the functionality can be achieved using Quarkus [extensions](https://quarkus.io/guides/writing-extensions).
See the [integration guide](https://quarkus.io/guides/cdi-integration) for more information.

## <a name="development-mode"></a> Dev mode
...
* [Transformation of unproxyable classes](#ability-to-handle-final-classes-and-methods)
* [Unused beans removal](#removing-unused-beans)
...
**📌 NOTE**\
La tabla de configuracion generada `quarkus-arc` se produce al construir la documentacion y no existe en el codigo fuente. Consulta la referencia de configuracion en https://quarkus.io/guides/all-config
