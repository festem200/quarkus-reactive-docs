...
> **Guia oficial:** <https://quarkus.io/guides/signals>  
> **Fuente:** `docs/src/main/asciidoc/signals.adoc` en [quarkusio/quarkus@3.38.2](https://github.com/quarkusio/quarkus/blob/3.38.2/docs/src/main/asciidoc/signals.adoc)  
...
* **Resolution**\
The extension provides a type-safe resolution model, inspired by the [CDI events](https://jakarta.ee/specifications/cdi/4.1/jakarta-cdi-spec-4.1#events).
* **Emission Modes**\
Inspired by the [Vert.x EventBus](https://vertx.io/docs/vertx-core/java/#event_bus), the API supports unicast (point-to-point) and request-reply patterns in addition to the traditional multicast (publish-subscribe) pattern.
* **Execution Model**\
Receivers are always executed asynchronously and support both blocking and non-blocking logic. The execution model is determined from the method return type and annotations such as `@Blocking`, `@NonBlocking`, and `@RunOnVirtualThread`, following Quarkus conventions.
...
Additional parameters may be declared and are treated as injection points -- the container will obtain and inject the corresponding bean instances when the receiver is invoked, just like [CDI observer methods](https://jakarta.ee/specifications/cdi/4.1/jakarta-cdi-spec-4.1#observes).
...
The return type is also used during [Receiver Resolution](#receiver-resolution) -- only receivers whose response type is assignable to the requested type are considered.
...
1. If the method is annotated with `@io.smallrye.common.annotation.RunOnVirtualThread`, the execution model is `VIRTUAL_THREAD`.
2. If the method is annotated with `@io.smallrye.common.annotation.Blocking`, the execution model is `BLOCKING`.
3. If the method is annotated with `@io.smallrye.common.annotation.NonBlocking`, the execution model is `NON_BLOCKING`.
4. If none of the above is present on the method, the declaring class is checked for the same annotations.
5. If no annotation is found, the default is derived from the method signature:
...
```java
Registration reg = receivers.newReceiver(OrderPlaced.class)
        .setQualifiers(new Urgent.Literal())                        // ①
...
The resolution rules are inspired by the [CDI observer resolution](https://jakarta.ee/specifications/cdi/4.1/jakarta-cdi-spec-4.1#observer_resolution) rules:
...
**📌 NOTE**\
La tabla de configuracion generada `quarkus-signals` se produce al construir la documentacion y no existe en el codigo fuente. Consulta la referencia de configuracion en https://quarkus.io/guides/all-config
