...
> **Guia oficial:** <https://quarkus.io/guides/vertx-reference>  
> **Fuente:** `docs/src/main/asciidoc/vertx-reference.adoc` en [quarkusio/quarkus@3.38.2](https://github.com/quarkusio/quarkus/blob/3.38.2/docs/src/main/asciidoc/vertx-reference.adoc)  
...
This guide is the companion to the [Using Eclipse Vert.x API from a Quarkus Application](vertx.md) guide.
...
**📌 NOTE**\
La tabla de configuracion generada `quarkus-vertx` se produce al construir la documentacion y no existe en el codigo fuente. Consulta la referencia de configuracion en https://quarkus.io/guides/all-config

See [Customize the Vert.x configuration](#customize-the-vertx-configuration) to configure the Vert.x instance using a programmatic approach.
...
|     |     |     |
| --- | --- | --- |
| API | Extension or Dependency | Documentation |
| AMQP Client | `io.quarkus:quarkus-messaging-amqp` (extension) | [Getting Started to Quarkus Messaging with AMQP](../04-mensajeria/amqp.md) |
| Circuit Breaker | `io.smallrye.reactive:smallrye-mutiny-vertx-circuit-breaker` (external dependency) | https://vertx.io/docs/vertx-circuit-breaker/java/ |
| Consul Client | `io.smallrye.reactive:smallrye-mutiny-vertx-consul-client` (external dependency) | https://vertx.io/docs/vertx-consul-client/java/ |
| DB2 Client | `io.quarkus:quarkus-reactive-db2-client` (extension) | [Reactive SQL Clients](../03-datos/reactive-sql-clients.md) |
| Kafka Client | `io.quarkus:quarkus-messaging-kafka` (extension) | [Apache Kafka Reference Guide](../04-mensajeria/kafka.md) |
| Mail Client | `io.quarkus:quarkus-mailer` (extension) | [Sending emails using SMTP](../04-mensajeria/mailer.md) |
| MQTT Client | `io.quarkus:quarkus-messaging-mqtt` (extension) | No guide yet |
| MS SQL Client | `io.quarkus:quarkus-reactive-mssql-client` (extension) | [Reactive SQL Clients](../03-datos/reactive-sql-clients.md) |
| MySQL Client | `io.quarkus:quarkus-reactive-mysql-client` (extension) | [Reactive SQL Clients](../03-datos/reactive-sql-clients.md) |
| Oracle Client | `io.quarkus:quarkus-reactive-oracle-client` (extension) | [Reactive SQL Clients](../03-datos/reactive-sql-clients.md) |
| PostgreSQL Client | `io.quarkus:quarkus-reactive-pg-client` (extension) | [Reactive SQL Clients](../03-datos/reactive-sql-clients.md) |
| RabbitMQ Client | `io.smallrye.reactive:smallrye-mutiny-vertx-rabbitmq-client` (external dependency) | https://vertx.io/docs/vertx-rabbitmq-client/java |
| Redis Client | `io.quarkus:quarkus-redis-client` (extension) | [Using the Redis Client](../03-datos/redis.md) |
| Web Client | `io.smallrye.reactive:smallrye-mutiny-vertx-web-client` (external dependency) | https://vertx.io/docs/vertx-web-client/java/ |
...
Refer to [Jackson configuration](../02-web-http/rest-json.md#json) to customize the mapping.
...
[Verticles](https://vertx.io/docs/vertx-core/java/#_verticles) is "a simple, scalable, actor-like deployment and concurrency model" provided by _Vert.x_.
...
Vert.x comes with a built-in [event bus](https://vertx.io/docs/vertx-core/java/#event_bus) that you can use from your Quarkus application.
...
```java
@ConsumeEvent("${my.consumer.address}")   // ①
public String consume(String name) {
...
Read [the virtual thread guide](virtual-threads.md) for more details.

### Use codecs

The [Vert.x Event Bus](https://vertx.io/docs/vertx-core/java/#event_bus) uses [codecs](https://vertx.io/docs/vertx-core/java/#_message_codecs) to _serialize_ and _deserialize_ message objects.
...
More detailed explanations about the configuration can be found on [the Vert.x SockJS Bridge documentation](https://vertx.io/docs/vertx-web/java/#_sockjs_event_bus_bridge).
...
**📌 NOTE**\
To use `io_uring`, refer to the [Use io_uring](#use-io_uring) section.

Vert.x is capable of using [Netty’s native transports](https://netty.io/wiki/native-transports.html), which offers performance improvements on specific platforms.
...
**❗ IMPORTANT**\
This will only work on platforms that support [Use native transports](#use-native-transports).

Enable the appropriate [Use native transports](#use-native-transports) and set the following environment property:
...
See [Use native transports](#use-native-transports) for details.
...
* [Why you should use io_uring for network I/O](https://developers.redhat.com/articles/2023/04/12/why-you-should-use-iouring-network-io): The main benefit of io_uring for network I/O is a modern asynchronous API that is straightforward to use and provides unified semantics for file and network I/O.
...
* [The Backend Revolution and Why io_uring Is So Important](https://dzone.com/articles/the-backend-revolution-or-why-io-uring-is-so-impor): The io_uring API uses two ring buffers for communication between application and kernel (hence the API name) and designed in a way that enables natural batching of requests and responses.
Besides, it provides a way to submit multiple requests in one system call, which can reduce overhead.
* [What exactly is io_uring?](https://stackoverflow.com/questions/61767702/what-exactly-is-io-uring): io_uring is a Linux kernel interface to efficiently allow you to send and receive data asynchronously.
It was originally designed to target block devices and files but has since gained the ability to work with things like network sockets.
...
<dl><dt><strong>📌 NOTE: Can I use io_uring on my Linux machine?</strong></dt><dd>
...
<dl><dt><strong>📌 NOTE: Troubleshooting</strong></dt><dd>

`io_uring` support is still experimental.
Check the [Netty io_uring FAQ](https://github.com/netty/netty-incubator-transport-io_uring#faq) if you see some odd behavior.
Also, the [netty io_uring was slower than epoll](https://github.com/netty/netty-incubator-transport-io_uring/issues/152) issue describes a few configuration mistakes.
</dd></dl>
