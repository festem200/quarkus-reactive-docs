...
> **Guia oficial:** <https://quarkus.io/guides/websockets-next-reference>  
> **Fuente:** `docs/src/main/asciidoc/websockets-next-reference.adoc` en [quarkusio/quarkus@3.38.2](https://github.com/quarkusio/quarkus/blob/3.38.2/docs/src/main/asciidoc/websockets-next-reference.adoc)  
...
The _WebSocket_ protocol, documented in the [RFC6455](https://datatracker.ietf.org/doc/html/rfc6455), establishes a standardized method for creating a bidirectional communication channel between a client and a server through a single TCP connection.
...
Both the [Server API](#server-api) and [Client API](#client-api) define _endpoints_ that are used to consume and send messages.
The endpoints are implemented as CDI beans and support injection.
Endpoints declare [_callback methods_](#callback-methods) annotated with `@OnTextMessage`, `@OnBinaryMessage`, `@OnPingMessage`, `@OnPongMessage`, `@OnOpen`, `@OnClose` and `@OnError`.
...
**📌 NOTE**\
The client API also includes [connectors](#client-connectors) that are used to configure and create new WebSocket connections.
...
**📌 NOTE**\
Client endpoints are used to consume and send messages. You’ll need the [connectors API](#client-connectors) to configure and open new WebSocket connections.
...
Subsequent invocations of [Callback methods](#callback-methods) within the same connection utilize the same session context.
...
See [When to subscribe to a `Uni` or `Multi`](#when-to-subscribe-to-a-uni-or-multi) to learn more about subscribing to the incoming `Multi`.
...
Items sent to the client are [serialized](#serialization-and-deserialization) except for the `String`, `io.vertx.core.json.JsonObject`, `io.vertx.core.json.JsonArray`, `io.vertx.core.buffer.Buffer`, and `byte[]` types.
...
```java
@OnTextMessage(
        codec = MyInputCodec.class, // ①
        outputCodec = MyOutputCodec.class // ②
Item find(Item item) {
        //....
}
```
...
A [ping message](https://datatracker.ietf.org/doc/html/rfc6455#section-5.5.2) may serve as a keepalive or to verify the remote endpoint.
A [pong message](https://datatracker.ietf.org/doc/html/rfc6455#section-5.5.3) is sent in response to a ping message and it must have an identical payload.
...
quarkus.websockets-next.client.auto-ping-interval=10 ②
```
...
Refer to the [HTTP guide](http-reference.md) for more details.
...
**📌 NOTE**\
Note that outside of these methods, the `WebSocketConnection` object is not available. However, it is possible to [list all open connections](#list-open-connections).
...
See [When to subscribe to a `Uni` or `Multi`](#when-to-subscribe-to-a-uni-or-multi) to learn more about subscribing to the `Uni`.
...
class MyBean {
  ...
  void logAllOpenConnections() {
     Log.infof("Open connections: %s", connections.listAll()); // ①
  }
}
```
...
class MyEndpoint {
  ...
  void open(WebSocketConnection connection) {
     connection.userData().put(TypedKey.forBoolean("isCool"), service.isCool(connection.pathParam("username"))); ①
  }
  
  @OnTextMessage
  String process(String message) {
     if (connection.userData().get(TypedKey.forBoolean("isCool"))) { ②
...
Any [Identity provider](https://quarkus.io/guides/security-identity-providers) can be used to convert authentication credentials on the initial HTTP request into a `SecurityIdentity` instance.
...
You should always prefer HTTP upgrade security unless you need to perform an action on error (see [Secure WebSocket endpoint callback methods](#secure-websocket-endpoint-callback-methods)) or a security check based on the payload (see [Secure server endpoints with permission checkers](#secure-server-endpoints-with-permission-checkers)).
...
See the [Authentication mechanisms in Quarkus](https://quarkus.io/guides/security-authentication-mechanisms#use-annotations-for-path-based-auth) guide for more information about selecting authentication mechanisms with annotations.
...
See also the [Custom HttpSecurityPolicy](https://quarkus.io/guides/security-authorize-web-endpoints-reference#custom-http-security-policy) section of the Authorization of web endpoints guide for more information about custom policies.
...
`jakarta.annotation.security.RolesAllowed` and other annotations listed in the [Supported security annotations](https://quarkus.io/guides/security-authorize-web-endpoints-reference#standard-security-annotations) documentation.
...
WebSocket endpoints can be secured with the [permission checkers](https://quarkus.io/guides/security-authorize-web-endpoints-reference#permission-checker).
We recommend to [Secure HTTP upgrade](#secure-http-upgrade) rather than individual endpoint methods. For example:
...
public class PermissionChecker {
    ...
    public boolean canGetPremiumProduct(SecurityIdentity securityIdentity) { ①
...
The [OIDC Bearer token authentication](https://quarkus.io/guides/security-oidc-bearer-token-authentication) expects that the bearer token is passed in the `Authorization` header during the initial HTTP handshake.
Java WebSocket clients such as [WebSockets Next Client](#client-api) and [Vert.x WebSocketClient](https://vertx.io/docs/vertx-core/java/#_websockets_on_the_client) support adding custom headers to the WebSocket opening handshake.
However, JavaScript clients that follow the [WebSockets API](https://websockets.spec.whatwg.org/#the-websocket-interface) do not support adding custom headers.
...
The JavaScript WebSocket client only allows to configure the HTTP [Sec-WebSocket-Protocol](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Sec-WebSocket-Protocol) request header for negotiating a sub-protocol.
If absolutely necessary, the `Sec-WebSocket-Protocol` header might be used as a carrier for custom headers, to provide a workaround the [WebSockets API](https://websockets.spec.whatwg.org/#the-websocket-interface) restrictions.
...
const socket = new WebSocket("wss://" + location.host + "/chat/" + username, ["bearer-token-carrier", quarkusHeaderProtocol]) ②
```
...
* Restrict supported Origins to trusted Origins only with the [CORS filter](security-cors.md#cors-filter).
...
public class Endpoint {
    ...
    String echo(RequestDto request) {
        if (request.metadata != null && request.metadata.token != null) {
            webSocketSecurity.updateSecurityIdentity(request.metadata.token); ①
        }
...
The [OIDC Bearer token authentication](https://quarkus.io/guides/security-oidc-bearer-token-authentication) mechanism has builtin support for the `SecurityIdentity` update.
...
As a direct consequence of the fact this extension reuses the _main_ HTTP server, all the relevant server configurations apply. See Refer to the [HTTP guide](http-reference.md#ssl) for more details.
...
```
For more information on Hibernate multitenancy, refer to the [hibernate documentation](https://quarkus.io/guides/hibernate-orm#multitenancy).
...
**📌 NOTE**\
Note that outside of these methods, the `WebSocketClientConnection` object is not available. However, it is possible to [list all open client connections](#list-open-client-connections).
...
class MyBean {
  ...
  void logAllOpenClinetConnections() {
     Log.infof("Open client connections: %s", connections.listAll()); ①
  }
}
```
...
class MyBean {
  ...
  void connectionOpened(@ObservesAsync @Open WebSocketClientConnection connection) { ①
     // This observer method is called when a connection is opened...
  }
}
```
...
To establish a TLS connection, you need to configure a _named_ configuration using the [TLS registry](../08-rendimiento-nativo/tls-registry-reference.md). This is typically done via configuration:
...
**📌 NOTE**\
La tabla de configuracion generada `quarkus-websockets-next` se produce al construir la documentacion y no existe en el codigo fuente. Consulta la referencia de configuracion en https://quarkus.io/guides/all-config
