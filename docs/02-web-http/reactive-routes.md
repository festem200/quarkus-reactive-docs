...
> **Guia oficial:** <https://quarkus.io/guides/reactive-routes>  
> **Fuente:** `docs/src/main/asciidoc/reactive-routes.adoc` en [quarkusio/quarkus@3.38.2](https://github.com/quarkusio/quarkus/blob/3.38.2/docs/src/main/asciidoc/reactive-routes.adoc)  
...
The code presented in this guide is available in this [GitHub repository](https://github.com/quarkusio/quarkus-quickstarts) under the [`reactive-routes-quickstart` directory](https://github.com/quarkusio/quarkus-quickstarts/tree/main/reactive-routes-quickstart)

**📌 NOTE**\
Reactive Routes were initially introduced to provide a reactive execution model for HTTP APIs on top of the [Quarkus Reactive Architecture](../01-fundamentos/quarkus-reactive-architecture.md).
With the introduction of [Quarkus REST (formerly RESTEasy Reactive)](rest.md), you can now implement reactive HTTP APIs and still use Jakarta REST annotations.
...
![http-architecture](../_assets/http-architecture.png)
...
3. The method gets a [`RoutingContext`](https://vertx.io/docs/apidocs/io/vertx/ext/web/RoutingContext.html) as a parameter. From the `RoutingContext` you can retrieve the HTTP request (using `request()`) and write the response using `response().end(...)`.
...
* The `path` - for routing by path, using the [Vert.x Web format](https://vertx.io/docs/vertx-web/java/#_capturing_path_parameters)
* The `regex` - for routing with regular expressions, see [for more details](https://vertx.io/docs/vertx-web/java/#_routing_with_regular_expressions)
...
You should read the [Virtual thread support reference](../01-fundamentos/virtual-threads.md#run-code-on-virtual-threads-using-runonvirtualthread) carefully and get acquainted with all the details.
...
<dl><dt><strong>📌 NOTE: Deprecation of `asJsonArray`</strong></dt><dd>
...
<dl><dt><strong>📌 NOTE: Deprecation of `asEventStream`</strong></dt><dd>
...
<dl><dt><strong>📌 NOTE: Deprecation of `asJsonStream`</strong></dt><dd>
...
If the request accepts JSON payload, the response follows the [Problem](https://opensource.zalando.com/problem/constraint-violation/) format.
...
Check the [Vert.x Web documentation](https://vertx.io/docs/vertx-web/java/#_basic_vert_x_web_concepts) to know more about the route registration, options, and available handlers.
...
You can add support for [OpenAPI](https://www.openapis.org/) and [Swagger UI](https://swagger.io/tools/swagger-ui/) by using the `quarkus-smallrye-openapi` extension.
...
Also see [the OpenAPI Guide](openapi-swaggerui.md).

### Adding MicroProfile OpenAPI Annotations

You can use [MicroProfile OpenAPI](https://github.com/eclipse/microprofile-open-api) to better document your schema,
...
For more information, see the [Swagger UI](openapi-swaggerui.md#dev-mode) guide.

Navigate to [localhost:8080/q/swagger-ui/](http://localhost:8080/q/swagger-ui/) and observe the Swagger UI screen:

![reactive-routes-guide-screenshot01](../_assets/reactive-routes-guide-screenshot01.png)
