...
> **Guia oficial:** <https://quarkus.io/guides/dev-ui>  
> **Fuente:** `docs/src/main/asciidoc/dev-ui.adoc` en [quarkusio/quarkus@3.38.2](https://github.com/quarkusio/quarkus/blob/3.38.2/docs/src/main/asciidoc/dev-ui.adoc)  
...
![Dev UI Layout](../_assets/devui_layout_light.png)
...
| --- | --- |
| **Extensions** | Displays all Quarkus extensions (used by the current application) as cards. Each card may provide configuration, documentation, or tools. |
| **Configuration** | Shows all configuration options (with current, default, and documentation info). Includes a config editor (text & form modes). |
| **Workspace** | A project file browser. Files can be opened and edited inline. Extensions may add workspace actions. |
| **Endpoints** | Lists all HTTP endpoints (REST, GraphQL, internal, etc.). Helps you inspect available routes in your app. |
| **Continuous Testing** | Monitors and controls continuous testing. View test results, rerun tests, or toggle testing state. |
| **Dev Services** | Displays information about automatically started services (e.g., databases, Kafka brokers). |
| **Build Metrics** | Provides performance metrics from the last build and reload cycle. |
| **Readme** | Renders your project’s `README.md`, if available. Useful for onboarding or project context. |
| **Dependencies** | Shows all runtime and deployment dependencies with dependency path exploration and search functionality. |
...
![Extension Card Anatomy](../_assets/devui_card_light.png)

Each card may include the following elements:

| Element | Description |
| --- | --- |
| **Logo** _(optional)_ |  An optional logo representing the extension or its domain |
| **Title** | The name of the extension as recognized in the Dev UI. |
| **Favorite** | Marks the card as a favorite. Favorited cards are always displayed first in the Extensions view for easier access. |
| **Guide** _(optional)_ | Links to the online Quarkus guide for the extension (if available). |
| **Description** | A short explanation of what the extension does or enables. |
| **Page Links** _(optional)_ | Interactive entries that navigate to custom pages contributed by the extension. A page link can also optionally contain a label that we discuss later. |
| **Configuration** | A shortcut to the configuration editor filtered to settings relevant to this extension. |
| **Underlying Library** _(optional)_ | Shows the version of the main library powering the extension (if any). |
| **Text** _(optional)_ | Displays informational text values (such as versions, counts, or status) as badge-style elements. Can show static, dynamic (fetched once), or streaming (continuously updated) values. |
| **More Details** | Opens a dialog with all information |
...
![Dev UI Settings](../_assets/devui_layout_settings.png)
...
This can be transitively or directly and is only needed in deployment module. This gives your processor access to the relative Build Items.
...
6. You can add an icon. All free [font-awesome](https://fontawesome.com/icons) icons are available.
...
If you find your icon at [Font awesome](https://fontawesome.com/search?o=r&m=free), you can map as follow: Example `<i class="fa-solid fa-house"></i>` will map to `font-awesome-solid:house`, so `fa` becomes `font-awesome` and for the icon name, remove the `fa-`.

##### Optional: Library Version

You can add an underlying library badge on the card. This is useful for cases where the extension is exposing a well known library. As example, the `quarkus-hibernate-orm` extension provides access to Hibernate. Showing the underlying Hibernate details could be useful information for the user:

![Dev UI Layout](../_assets/devui_hibernate_card_light.png)
...
![dev-ui-extension-card-label-v2](../_assets/dev-ui-extension-card-label-v2.png)
...
public void createFooterLog(BuildProducer<FooterLogBuildItem> footerLogProducer){
    footerLogProducer.produce(new FooterLogBuildItem("My Extension Log", () -> {// ②
        return createLogPublisher();// ③
    }));
}
```
1. Always make sure that this build step is only run when in local dev mode.
...
![Dev UI Jokes](../_assets/devui_jokes_light.png)
...
```java
    ...
    void createMCPSettingsTab(BuildProducer<SettingPageBuildItem> settingPageProducer) {
        ...
        SettingPageBuildItem mcpSettingTab = new SettingPageBuildItem();// ②
...
This page will be loaded as the content for that tab in the settings dialog
...
Lastly, you can add pages that is not listed anywhere, but added to the Router, and can be navigated to. This is useful if you are building very complex pages that navigates to more pages.
...
```java
    ...
    void createMCPUnlistedPages(BuildProducer<UnlistedPageBuildItem> unlistedPageProducer) {
        ...
        UnlistedPageBuildItem mcpOtherPages = new UnlistedPageBuildItem(); // ②
...
This page will be registered to the Router and can be navigated to by a normal link or using the Router:
...
let unlistedPages = this.routerController.getPagesForCurrentNamespace(); // ③
// or if you not in your own space
// let unlistedPages = this.routerController.getPagesForNamespace("your namespace"); // ④
...
Dev UI uses [Lit](https://lit.dev/) to make building these web components easier. You can read more about Web Components and Lit:
...
5. Styles can reference globally defined CSS variables to make sure your page renders correctly, especially when switching between light and dark mode. You can find all CSS variables in the Vaadin documentation ([Color](https://vaadin.com/docs/latest/styling/lumo/lumo-style-properties/color), [Sizing and Spacing](https://vaadin.com/docs/latest/styling/lumo/lumo-style-properties/size-space), etc.)
...
8. `connectedCallback` is a method from Lit that gets called when this component is connected to the DOM. This is a good place to do 'page load' type things, like fetching data from the backend (we will discuss this later)
9. `disconnectedCallback` is a method from Lit that gets called when this component is disconnected from the DOM. This is a good place to do any clean up.
...
11. You can use the `html` function from `Lit`, which gives you a template language to output the HTML you want. Once the template is created, you must only set or change the properties to re-render the page content. Read more about [Lit html](https://lit.dev/docs/components/rendering/)
...
[Vaadin Web Components](https://vaadin.com/docs/latest/components): Dev UI makes extensive usage of Vaadin web components as UI Building blocks.

###### Qomponent
[Qomponent](https://github.com/qomponent): A few custom build components that can be used. 
...
This component uses the above mentioned code block from qomponent, that is built with [code-mirror](https://codemirror.net/), but adds the automatic theme state when switching themes.
...
See the [@qomponent/qui-code-block](https://github.com/qomponent/qui-code-block) for more details.
...
```
[Example code](https://github.com/quarkusio/quarkus/blob/b84f13bb1a94073900328829e0b2224903179baf/extensions/devui/resources/src/main/resources/dev-ui/qwc/qwc-server-log.js#L334)

##### Using internal controllers

Some [internal controllers](https://github.com/quarkusio/quarkus/tree/main/extensions/devui/resources/src/main/resources/dev-ui/controller) are available to make certain things easier:
...
See the source of this controller [here](https://github.com/quarkusio/quarkus/blob/main/extensions/devui/resources/src/main/resources/dev-ui/controller/notifier.js).

Example usage:

![dev-ui-controller-notifier](../_assets/dev-ui-controller-notifier.gif)
...
You can find all the valid positions [here](https://vaadin.com/docs/latest/components/notification/#position).

[Example code](https://github.com/quarkiverse/quarkus-jokes/blob/f572ed6f949de0c0b8cbfa99d7389ab5168fea65/deployment/src/main/resources/dev-ui/qwc-jokes-vaadin.js#L374)
...
[Example code](https://github.com/quarkusio/quarkus/blob/main/extensions/devui/resources/src/main/resources/dev-ui/qwc/qwc-footer.js)
...
![dev-ui-storage-settings](../_assets/dev-ui-storage-settings.png)
...
![dev-ui-log-control-v2](../_assets/dev-ui-log-control-v2.png)
...
[Example code](https://github.com/quarkusio/quarkus/blob/main/extensions/devui/resources/src/main/resources/dev-ui/qwc/qwc-server-log.js)

###### Router

The router is mostly used internally. This uses [Vaadin Router](https://github.com/vaadin/router) under the covers to route URLs to the correct page/section within the SPA. It will update the navigation and allow history (back button). This also creates the sub-menu available on extensions that have multiple pages.

See the [controller](https://github.com/quarkusio/quarkus/blob/main/extensions/devui/resources/src/main/resources/dev-ui/controller/router-controller.js) for some methods that might be useful.

##### Global State

The properties on a Web component page is scope for that page/webcomponent. There is also a few global states available that might be useful in your component. Dev UI uses [LitState](https://gitaarik.github.io/lit-state/build/) for this. LitState automatically re-renders your LitElement components, when a shared app state variable they use changes. It’s like LitElement’s properties, but then shared over multiple components. 
...
export class QwcExtensionPage extends observeState(LitElement) {  // ③
```
...
You can see all the properties of connection state [here](https://github.com/quarkusio/quarkus/blob/main/extensions/devui/resources/src/main/resources/dev-ui/state/connection-state.js)
...
You can see all the properties of theme state [here](https://github.com/quarkusio/quarkus/blob/main/extensions/devui/resources/src/main/resources/dev-ui/state/theme-state.js)
...
render() {
    if(assistantState.current.isConfigured){
        return html`<div class="assistantfeature">
                        <span> Magic happens here</span>
                    </div>`;
    }
}
```

You can see all the properties of assistant state [here](https://github.com/quarkusio/quarkus/blob/main/extensions/devui/resources/src/main/resources/dev-ui/state/assistant-state.js)
...
[here](https://github.com/quarkusio/quarkus/blob/main/extensions/devui/resources/src/main/resources/dev-ui/state/devui-state.js)
...
![dev-ui-extension-openapi-v2](../_assets/dev-ui-extension-openapi-v2.png)
...
By default, even external links will render inside (embedded) in Dev UI. In the case of HTML, the page will be rendered, and any other content will be shown using [code-mirror](https://codemirror.net/) to markup the media type. For example, the OpenAPI schema document in YAML format:

![dev-ui-extension-openapi-embed-v2](../_assets/dev-ui-extension-openapi-embed-v2.png)
...
If the RPC method accepts parameters, you can add these to the method
`.dynamicUrlJsonRPCMethodName("yourJsonRPCMethodName", Map.of("parameter-name", "parameter-value))`.
The RPC method lookup defaults to the extension namespace. RPC methods from other extension namespaces can be used by prefixing the method name with the namespace : `.dynamicUrlJsonRPCMethodName("otherNamespace:jsonRPCMethodName")`.

For example, the following Page definition will call the `devui-dev-services:devServicesConfig` JsonRPC method at runtime to get the URL to the dev service named `my-dev-service`:
...
![dev-ui-raw-page-v2](../_assets/dev-ui-raw-page-v2.png)
...
![dev-ui-table-page-v2](../_assets/dev-ui-table-page-v2.png)
...
All communication to the backend happens with [JsonRPC](https://www.jsonrpc.org/) over web socket. Dev UI makes this easy to use for extension developers, and you do not really need to get into the details of either JsonRPC or Web Sockets.
...
[Example code](https://github.com/quarkiverse/quarkus-jokes/blob/main/runtime/src/main/java/io/quarkus/jokes/runtime/JokesJsonRPCService.java)
...
```java
@BuildStep
JsonRPCProvidersBuildItem createJokesJsonRPCService() {// ①
...
1. Note the method `getJoke` corresponds to the method in your Java Service. This method returns a [Promise](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise) with the JsonRPC result.
...
[Example code](https://github.com/quarkiverse/quarkus-jokes/blob/main/deployment/src/main/resources/dev-ui/qwc-jokes-list.js)
...
[Example code](https://github.com/quarkiverse/quarkus-jokes/blob/f572ed6f949de0c0b8cbfa99d7389ab5168fea65/runtime/src/main/java/io/quarkus/jokes/runtime/JokesJsonRPCService.java#L37)
...
![dev-ui-workspace](../_assets/dev-ui-workspace.png)
...
The input your function receives is:
...
* raw: This will be used as is (text)
...
![dev-ui-jsonrpc-log-v2](../_assets/dev-ui-jsonrpc-log-v2.png)
...
4. Your tag definition has to be the filename without the `.js`
