...
> **Guia oficial:** <https://quarkus.io/guides/getting-started-testing>  
> **Fuente:** `docs/src/main/asciidoc/getting-started-testing.adoc` en [quarkusio/quarkus@3.38.2](https://github.com/quarkusio/quarkus/blob/3.38.2/docs/src/main/asciidoc/getting-started-testing.adoc)  
...
**📌 NOTE**\
Quarkus supports Continuous testing, but this is covered by the [Continuous Testing Guide](continuous-testing.md).
...
Because we are using JUnit, the version of the [Surefire Maven Plugin](https://maven.apache.org/surefire/maven-surefire-plugin/)
...
<dl><dt><strong>💡 TIP: Changing the test port</strong></dt><dd>
...
JUnit [@Nested tests](https://docs.junit.org/current/writing-tests/nested-tests.html) are useful for structuring more complex test scenarios.
...
**📌 NOTE**\
Contrary to what happens with `@InjectMock`, when using `@InjectSpy` for a bean that used CDI observers, these observers are not disabled but executed as per the normal CDI rules.
...
[CDI `@Alternative` mechanism.](#cdi-alternative-mechanism).

### Mocking with Panache

If you are using the `quarkus-hibernate-orm-panache` or `quarkus-mongodb-panache` extensions, check out the [Hibernate ORM with Panache Mocking](https://quarkus.io/guides/hibernate-orm-panache#mocking) and [MongoDB with Panache Mocking](../03-datos/mongodb-panache.md#mocking) documentation for the easiest way to mock your data access.
...
Common cases include starting docker containers using [Testcontainers](https://www.testcontainers.org/) (an example of which can be found [here](https://github.com/quarkusio/quarkus/blob/main/test-framework/keycloak-server/src/main/java/io/quarkus/test/keycloak/server/KeycloakTestResourceLifecycleManager.java)),
or starting a mock HTTP server using [Wiremock](https://wiremock.org/) (an example of which can be found [here](https://github.com/geoand/quarkus-test-demo/blob/main/src/test/java/org/acme/getting/started/country/WiremockCountries.java)).
...
This is covered in the [Native Executable Guide](../08-rendimiento-nativo/building-native-image.md).
...
More information can be found in the [Testing the native executable Guide](../08-rendimiento-nativo/building-native-image.md#testing-the-native-executable).
...
This means that `@QuarkusIntegrationTest` works out of the box with containers launched via [Dev Services](dev-services.md), but it also means that it enables using [QuarkusTestLifecycleManager](#starting-services-before-the-quarkus-application-starts) resources that launch additional containers.
...
<details>
<summary>Source set configuration example</summary>
...
* `java.util.logging.manager` (see [Logging Guide](../07-observabilidad/logging.md))
* `maven.home` (only if there are any custom settings in `${maven.home}/conf/settings.xml`, see [Maven Guide](../10-extras/maven-tooling.md))
...
By default, tests should just work with [Dev Services](dev-services.md), however from some use cases you may need access to
...
@QuarkusTest
@Tag("security")
public @interface QuarkusSecurityTest {   ①
}
```
...
You can find more examples and hints in the [testing components reference guide](testing-components.md).
