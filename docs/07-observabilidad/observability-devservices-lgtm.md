...
> **Guia oficial:** <https://quarkus.io/guides/observability-devservices-lgtm>  
> **Fuente:** `docs/src/main/asciidoc/observability-devservices-lgtm.adoc` en [quarkusio/quarkus@3.38.2](https://github.com/quarkusio/quarkus/blob/3.38.2/docs/src/main/asciidoc/observability-devservices-lgtm.adoc)  
> **Version documentada:** Quarkus 3.38.2 · **Sincronizado:** 2026-08-17 · **Licencia:** Apache-2.0

This Dev Service provides the [Grafana OTel-LGTM](https://github.com/grafana/docker-otel-lgtm), an `all-in-one` Docker image containing an [OpenTelemetry Collector](https://opentelemetry.io/docs/collector) receiving and then forwarding telemetry data to Prometheus (metrics), Tempo (traces) and Loki (logs).
This data can then be visualized by [Grafana](https://github.com/grafana/grafana). The LGTM abbreviation stands for:

* L -> [Loki](https://github.com/grafana/loki) (logs)
* G -> [Grafana](https://github.com/grafana/grafana) (metrics visualization)
* T -> [Tempo](https://github.com/grafana/tempo) (traces)
* M -> [Mimir](https://grafana.com/oss/mimir) (long term storage for Prometheus)
...
The [Micrometer Quarkus extension](https://quarkus.io/guides/telemetry-micrometer) provides metrics from automatic instrumentation implemented in Quarkus and its extensions.
...
The [Quarkiverse Micrometer OTLP registry](https://docs.quarkiverse.io/quarkus-micrometer-registry/dev/micrometer-registry-otlp.html) will output data using the OpenTelemetry OTLP protocol to the Grafana LGTM Dev Service. This will **push** data out of the service:
...
When using the [Micrometer’s](https://micrometer.io/) Quarkiverse OTLP registry to push metrics to Grafana OTel LGTM, the `quarkus.micrometer.export.otlp.url` property is automatically set to OTel collector endpoint as seen from the outside of the Docker container.

### OpenTelemetry

With OpenTelemetry, metrics, traces and logs can be created and sent to the Grafana LGTM Dev Service.

By default, the [OpenTelemetry extension](https://quarkus.io/guides/opentelemetry) will produce [traces](https://quarkus.io/guides/opentelemetry-tracing). [Metrics](https://quarkus.io/guides/opentelemetry-metrics) and [logs](https://quarkus.io/guides/opentelemetry-logging) must be enabled separately.
...
![dev-ui-observability-card](../_assets/dev-ui-observability-card.png)
...
![observability-grafana-tempo](../_assets/observability-grafana-tempo.png)

For logs, select the `loki` data source and query for data:

![observability-grafana-loki](../_assets/observability-grafana-loki.png)

### The dashboards

The Dev Service includes a set of dashboards.

![observability-grafana-dashboards](../_assets/observability-grafana-dashboards.png)
