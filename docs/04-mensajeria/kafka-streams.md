...
> **Guia oficial:** <https://quarkus.io/guides/kafka-streams>  
> **Fuente:** `docs/src/main/asciidoc/kafka-streams.adoc` en [quarkusio/quarkus@3.38.2](https://github.com/quarkusio/quarkus/blob/3.38.2/docs/src/main/asciidoc/kafka-streams.adoc)  
...
It is recommended, that you have read the [Kafka quickstart](https://github.com/quarkusio/quarkus-quickstarts/tree/main/kafka-quickstart) before.
...
By exposing a Kafka Streams [interactive query](https://kafka.apache.org/documentation/streams/developer-guide/interactive-queries),
...
![kafka-streams-guide-architecture](../_assets/kafka-streams-guide-architecture.png)
...
The solution is located in the `kafka-streams-quickstart` [directory](https://github.com/quarkusio/quarkus-quickstarts/tree/main/kafka-streams-quickstart).
...
* If using Powershell, wrap `-D` parameters in double quotes e.g. `"-DprojectArtifactId=kafka-streams-quickstart-producer"`
...
```bash
./gradlew addExtension --extensions='quarkus-messaging-kafka'
```
...
More details about the different configuration options are available on the [Producer configuration](https://kafka.apache.org/documentation/#producerconfigs) and [Consumer configuration](https://kafka.apache.org/documentation/#consumerconfigs) section from the Kafka documentation.
...
**💡 TIP**\
Alternatively, you can use `kafka.bootstrap.servers` instead of `quarkus.kafka-streams.bootstrap-servers` as you did in the _generator_ project above.
...
but Kafka Streams provides you with [an API](https://kafka.apache.org/documentation/streams/developer-guide/interactive-queries#querying-remote-state-stores-for-the-entire-app) to obtain the information which node is hosting a given key.
...
![kafka-streams-guide-architecture-distributed](../_assets/kafka-streams-guide-architecture-distributed.png)
...
Refer to the Quarkus guides on [Micrometer](../07-observabilidad/telemetry-micrometer.md),
and [SmallRye Health](../06-resiliencia/smallrye-health.md) to learn more.

## Configuration Reference

**📌 NOTE**\
La tabla de configuracion generada `quarkus-kafka-streams` se produce al construir la documentacion y no existe en el codigo fuente. Consulta la referencia de configuracion en https://quarkus.io/guides/all-config
