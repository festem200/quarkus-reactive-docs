...
> **Guia oficial:** <https://quarkus.io/guides/messaging>  
> **Fuente:** `docs/src/main/asciidoc/messaging.adoc` en [quarkusio/quarkus@3.38.2](https://github.com/quarkusio/quarkus/blob/3.38.2/docs/src/main/asciidoc/messaging.adoc)  
...
![Quarkus Messaging](../_assets/messaging-quarkus.png)
...
* [**Messaging - Kafka Connector**](kafka.md)
* [**Messaging - Pulsar Connector**](pulsar.md)
* [**Messaging - RabbitMQ Connector**](rabbitmq-reference.md)
* [**Messaging - AMQP 1.0 Connector**](amqp-reference.md)
...
* [Nats Jetstream Connector](https://docs.quarkiverse.io/quarkus-reactive-messaging-nats-jetstream/dev/index.html)
* [Solace Messaging Connector](https://solaceproducts.github.io/solace-quarkus/)
* [Reactive HTTP and WebSocket Connector](https://quarkus.io/extensions/io.quarkiverse.reactivemessaging.http/quarkus-reactive-messaging-http/)
* AWS SQS Connector
...
* [Kafka Streams Extension](https://quarkus.io/guides/kafka-streams)
* [RabbitMQ Client](https://docs.quarkiverse.io/quarkus-rabbitmq-client/dev/index.html)
* [HiveMQ Client](https://docs.quarkiverse.io/quarkus-hivemq-client/dev/index.html)
* [Artemis Core & JMS](https://docs.quarkiverse.io/quarkus-artemis/dev/quarkus-artemis-jms.html)
* [Google Cloud Pubsub](https://docs.quarkiverse.io/quarkus-google-cloud-services/main/pubsub.html)
...
You can read more on supported method signatures in the [SmallRye Reactive Messaging – Supported signatures](https://smallrye.io/smallrye-reactive-messaging/latest/concepts/signatures/).
...
For more control, using [Mutiny](https://smallrye.io/smallrye-mutiny/latest/) APIs, you can use the `MutinyEmitter` emitter interface:
...
You can read more on the emitters and channels in the [SmallRye Reactive Messaging – Emitter and Channels](https://smallrye.io/smallrye-reactive-messaging/latest/concepts/emitter/) documentation.
...
The Quarkus Kafka connector allows [access to the underlying client](kafka.md#kafka-bare-clients) to pause/resume the consumption.
...
Using [Mutiny APIs](https://smallrye.io/smallrye-mutiny/latest/) in incoming and outgoing signatures allow you to process the stream of messages:
...
Quarkus Messaging sits on top of the [reactive engine](../01-fundamentos/quarkus-reactive-architecture.md#engine) of Quarkus and leverages [Eclipse Vert.x](https://vertx.io/) to dispatch messages for processing.
...
Please refer to the dedicated [Quarkus Virtual Thread support](messaging-virtual-threads.md) guide for more information.
...
| --- | --- |
| @Incoming("source") void process(String payload) | Worker-threads |
| @Incoming("source") Uni&lt;Void> process(String payload) | Event-loop |
| @Incoming("source") CompletionStage&lt;Void> process(Message&lt;String> message) | Event-loop |
| @Incoming("source") @Outgoing("sink") Multi&lt;R> process(Multi&lt;T> in) | Stream-processing methods are executed at startup, then each message is dispatched on event loop. |

Fine-grained control over the execution model is possible using annotations:

* [`@Blocking`](https://javadoc.io/doc/io.smallrye.reactive/smallrye-reactive-messaging-api/latest/io/smallrye/reactive/messaging/annotations/Blocking.html) will force the method to be executed on a worker thread pool.
...
[message context](https://smallrye.io/smallrye-reactive-messaging/latest/concepts/message-context).
...
To learn more about context propagation in Quarkus and Mutiny, refer to the [Context Propagation](../01-fundamentos/context-propagation.md) guide.
...
Then the request-scoped bean can be accessed in the message processing stage, regardless of the [Execution Model](#execution-model):
...
For more information check the [Context Propagation](../01-fundamentos/context-propagation.md#context-propagation-for-cdi) guide.
</dd></dl>
...
The [Micrometer extension](../07-observabilidad/telemetry-micrometer.md) exposes these metrics.
...
When the [OpenTelemetry extension](../07-observabilidad/opentelemetry.md) is present, outgoing messages propagate the current tracing span.
...
Some messaging extensions integrate with the [Quarkus TLS Registry](../08-rendimiento-nativo/tls-registry-reference.md) to configure the underlying client.
...
You can read more about Dev Services in the [Dev Services](../09-testing/dev-services.md) guide, including a list of Dev Services provided by platform extensions.
...
public class InMemoryConnectorLifecycleManager implements QuarkusTestResourceLifecycleManager {
    ...
    public Map<String, String> start() {
        ...
        Map<String, String> props1 = InMemoryConnector.switchIncomingChannelsToInMemory("words-in");   // ①
        Map<String, String> props2 = InMemoryConnector.switchOutgoingChannelsToInMemory("uppercase");  // ②
        Map<String, String> props3 = InMemoryConnector.switchOutgoingChannelsToInMemory("words-out");  // ③
        env.putAll(props1);
        env.putAll(props2);
        env.putAll(props3);
        return env;  // ④
...
class MyMessagingApplicationTest {
    ...
    @Test
    void test() {
        InMemorySink<String> wordsOut = connector.sink("words-out"); // ②
        InMemorySource<String> wordsIn = connector.source("words-in"); // ③
        InMemorySink<String> uppercaseOut = connector.sink("uppercase"); // ④

        app.sendMessage("Hello"); // ⑤
        assertEquals("Hello", wordsOut.received().get(0).getPayload()); // ⑥

        wordsIn.send("Bonjour"); // ⑦
        await().untilAsserted(() -> assertEquals("BONJOUR", uppercaseOut.received().get(0).getPayload())); // ⑧
    }
}
...
[Channel decorators](https://smallrye.io/smallrye-reactive-messaging/latest/concepts/decorators/) is a way to intercept and decorate the reactive streams corresponding to messaging channels.
...
If you want to go further, you can check the [SmallRye Reactive Messaging](http://smallrye.io/smallrye-reactive-messaging/) documentation,
