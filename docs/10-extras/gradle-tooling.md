...
> **Guia oficial:** <https://quarkus.io/guides/gradle-tooling>  
> **Fuente:** `docs/src/main/asciidoc/gradle-tooling.adoc` en [quarkusio/quarkus@3.38.2](https://github.com/quarkusio/quarkus/blob/3.38.2/docs/src/main/asciidoc/gradle-tooling.adoc)  
...
To scaffold a Gradle project you can either use the [Quarkus CLI](cli-tooling.md) or the Quarkus Maven plugin:
...
_For more information about how to install the Quarkus CLI and use it, please refer to [the Quarkus CLI guide](cli-tooling.md)._
...
**📌 NOTE**\
If you just launch `mvn io.quarkus.platform:quarkus-maven-plugin:3.38.2:create` the Maven plugin asks
...
If you prefer to use a standalone Gradle installation, please use Gradle 9.6.0.
</dd></dl>
...
[INFO] [org.gradle.process.internal.DefaultExecHandle] Starting process 'command '/home/<username>/jdk/bin/java''. Working directory: /home/<username>/gradle-app/build/classes/java/main Command: /home/<username>/jdk/bin/java -Dquarkus.console.basic=true -Dio.quarkus.force-color-support=true -javaagent:/home/<username>/.m2/repository/io/quarkus/quarkus-class-change-agent/3.38.2/quarkus-class-change-agent-3.38.2.jar -Dquarkus-internal.serialized-app-model.path=/home/<username>/gradle-app/build/tmp/quarkusDev/quarkus-app-model.dat -Dquarkus-internal-test.serialized-app-model.path=/home/<username>/gradle-app/build/tmp/quarkusDev/quarkus-app-test-model.dat -XX:TieredStopAtLevel=1 -agentlib:jdwp=transport=dt_socket,address=localhost:5005,server=y,suspend=n --add-opens java.base/java.io=ALL-UNNAMED --add-opens java.base/java.nio=ALL-UNNAMED --enable-native-access=ALL-UNNAMED --add-modules=jdk.incubator.vector --enable-preview -Djava.util.logging.manager=org.jboss.logmanager.LogManager -jar /home/<username>/gradle-app/build/gradle-app-dev.jar
```
...
2. Set `quarkus.native.builder-image` property to `quay.io/quarkus/ubi9-quarkus-mandrel-builder-image:jdk-21`
...
Note also that starting Quarkus 3.19, the default _builder_ images are based on UBI 9. To use the previous UBI 8 based images, you can use the pick an image from the [quay.io repository](https://quay.io/repository/quarkus/ubi-quarkus-mandrel-builder-image?tab=tags).
</dd></dl>
...
Alternatively, there is some unofficial [Gradle Jandex plugins](https://plugins.gradle.org/search?term=jandex) that can be used instead of the `META-INF/beans.xml` file.
...
See [JavaForkOptions](https://docs.gradle.org/current/javadoc/org/gradle/process/JavaForkOptions.html)
for details.
...
[Gradle’s build cache](https://docs.gradle.org/current/userguide/build_cache.html) is a very efficient mechanism to
improve the overall build runtime, by reusing previously generated outputs (see
[Incremental build](https://docs.gradle.org/current/userguide/incremental_build.html) for technical details).
...
[Gradle’s GitHub action](https://github.com/gradle/gradle-build-action) or directly/manually GitHub’s
[GitHub’s cache action](https://github.com/actions/cache).
...
**💡 TIP**\
The Quarkus Gradle plugin detects a _CI environment_, if the `CI` environment variable is present.
...
[Gradle’s configuration cache](https://docs.gradle.org/current/userguide/configuration_cache.html) enabled, but
...
`disableQuarkusComponentVariants` project property was introduced in Quarkus 3.25.1 to control whether Quarkus component variants should be added to certain dependencies of a project to enable [Quarkus conditional](https://quarkus.io/guides/conditional-extension-dependencies) and build time (deployment) dependencies. Quarkus component variants are enabled by default since 3.25.1.
