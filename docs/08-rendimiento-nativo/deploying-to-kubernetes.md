...
> **Guia oficial:** <https://quarkus.io/guides/deploying-to-kubernetes>  
> **Fuente:** `docs/src/main/asciidoc/deploying-to-kubernetes.adoc` en [quarkusio/quarkus@3.38.2](https://github.com/quarkusio/quarkus/blob/3.38.2/docs/src/main/asciidoc/deploying-to-kubernetes.adoc)  
> **Version documentada:** Quarkus 3.38.2 · **Sincronizado:** 2026-08-17 · **Licencia:** Apache-2.0

Quarkus offers the ability to automatically generate Kubernetes resources based on sane defaults and user-supplied configuration using [dekorate](https://github.com/dekorateio/dekorate/).
...
Finally, when either one of container image extensions is present (see the [container image guide](container-image.md) for more details), Quarkus has the ability to create a container image and push it to a registry **before** deploying the application to the target platform.
...
Finally, the Kubernetes job will be launched every time it is installed in Kubernetes. You can know more about how to run Kubernetes jobs in this [link](https://kubernetes.io/docs/concepts/workloads/controllers/job/#running-an-example-job).

You can configure the rest of the Kubernetes Job configuration using the properties under `quarkus.kubernetes.job.xxx` (see [link](deploying-to-kubernetes.md#quarkus-kubernetes_quarkus-kubernetes-job-parallelism)).
...
**❗ IMPORTANT**\
CronJob resources require the [Cron](https://en.wikipedia.org/wiki/Cron) expression to specify when to launch the job via the property `quarkus.kubernetes.cron-job.schedule`. If not provide, the build will fail.

You can configure the rest of the Kubernetes CronJob configuration using the properties under `quarkus.kubernetes.cron-job.xxx` (see [link](deploying-to-kubernetes.md#quarkus-kubernetes_quarkus-kubernetes-cron-job-parallelism)).
...
The generated manifests use the Kubernetes [recommended labels](https://kubernetes.io/docs/concepts/overview/working-with-objects/common-labels).
...
More information about the health extension can be found in the relevant [guide](../06-resiliencia/smallrye-health.md).
...
### Add hostAliases
To add entries to a Pod’s `/etc/hosts` file (more information can be found in [Kubernetes documentation](https://kubernetes.io/docs/concepts/services-networking/add-entries-to-pod-etc-hosts-with-host-aliases/)), just apply the following configuration:
...
### Add nodeSelector
To add a nodeSelector in the generated `Deployment` (more information can be found in [Kubernetes documentation](https://kubernetes.io/docs/tasks/configure-pod-container/assign-pods-nodes)), just apply the following configuration:
...
CPU & Memory limits and requests can be applied to a `Container` (more info in [Kubernetes documentation](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/)) using the following configuration:
...
Kubernetes exposes applications using [Ingress resources](https://kubernetes.io/docs/concepts/services-networking/ingress). To generate the Ingress resource, just apply the following configuration:
...
To secure the incoming connections, Kubernetes allows enabling [TLS](https://kubernetes.io/docs/concepts/services-networking/ingress/#tls) within the Ingress resource by specifying a Secret that contains a TLS private key and certificate. You can generate a secured Ingress resource by simply adding the "tls.secret-name" properties:
...
More information about how to create the secret in [here](https://kubernetes.io/docs/concepts/services-networking/ingress/#tls).
</dd></dl>
...
In some scenarios, it’s necessary to generate additional [RBAC](https://kubernetes.io/docs/reference/access-authn-authz/rbac/) resources that are used by Kubernetes to grant or limit access to other resources. For example, in our use case, we are building [a Kubernetes operator](https://kubernetes.io/docs/concepts/extend-kubernetes/operator/#operators-in-kubernetes) that needs to read the list of the installed deployments. To do this, we would need to assign a service account to our operator and link this service account with a role that grants access to the Deployment resources. Let’s see how to do this using the `quarkus.kubernetes.rbac` properties:

```properties
# Generate the Role resource with name "my-role" ①
...
# Generate ServiceAccount resource with name "my-service-account" in namespace "my_namespace" ②
...
```properties
# Generate ClusterRole resource with name "my-cluster-role" ①
...
quarkus.kubernetes.rbac.cluster-role-bindings.my-cluster-role-binding.role-name=my-cluster-role ②
```
...
[Minikube](https://github.com/kubernetes/minikube) is quite popular when a Kubernetes cluster is needed for development purposes. To make the deployment to Minikube
...
To control the [nodePort](https://kubernetes.io/docs/concepts/services-networking/service/#nodeport) that is used in this case, users can set `quarkus.kubernetes.node-port`.
...
#### Deploying to Kind
[Kind](https://kind.sigs.k8s.io/) is another popular tool used as a Kubernetes cluster for development purposes. To make the deployment to Kind
...
**📌 NOTE**\
La tabla de configuracion generada `quarkus-kubernetes_quarkus.kubernetes` se produce al construir la documentacion y no existe en el codigo fuente. Consulta la referencia de configuracion en https://quarkus.io/guides/all-config
...
**📌 NOTE**\
La tabla de configuracion generada `quarkus-kubernetes-client` se produce al construir la documentacion y no existe en el codigo fuente. Consulta la referencia de configuracion en https://quarkus.io/guides/all-config
...
See further information in [Deploying to OpenShift](https://quarkus.io/guides/deploying-to-openshift).
...
**📌 NOTE**\
Quarkus also provides the [OpenShift](https://quarkus.io/guides/deploying-to-openshift) extension. This extension is basically a wrapper around the Kubernetes extension and
...
**📌 NOTE**\
La tabla de configuracion generada `quarkus-kubernetes_quarkus.openshift` se produce al construir la documentacion y no existe en el codigo fuente. Consulta la referencia de configuracion en https://quarkus.io/guides/all-config
...
**📌 NOTE**\
La tabla de configuracion generada `quarkus-kubernetes_quarkus.knative` se produce al construir la documentacion y no existe en el codigo fuente. Consulta la referencia de configuracion en https://quarkus.io/guides/all-config
...
* [Docker](container-image.md#docker)
* [Jib](container-image.md#jib)
* [s2i](container-image.md#s2i)
...
Finally, all you need to do is to configure your favorite IDE to attach the java agent process that is forwarded to `localhost:5005` and start to debug your application. For example, in IntelliJ IDEA, you can follow [this tutorial](https://www.jetbrains.com/help/idea/tutorial-remote-debug.html) to debug remote applications.
  ...
  template:
    ...
    spec:
      ...
      - env:
        - name: "FOO" ③
          value: "BAR"
        image: "<<yourDockerUsername>>/kubernetes-quickstart:1.0.0-SNAPSHOT" ④
        imagePullPolicy: "Always"
        name: "kubernetes-quickstart"
        ports:
        - containerPort: 8080 ⑤
...
## Service Binding <a name="service_binding"></a>

**⚠️ WARNING**\
Service Binding support has been deprecated

Quarkus supports the [Service Binding Specification for Kubernetes](https://github.com/servicebinding/spec) to bind services to applications.

Specifically, Quarkus implements the [Workload Projection](https://github.com/servicebinding/spec#workload-projection) part of the specification, therefore allowing applications to bind to services, such as a Database or a Broker, without the need for user configuration.
...
The [Service Binding Operator](https://github.com/redhat-developer/service-binding-operator) is an Operator that implements [Service Binding Specification for Kubernetes](https://github.com/servicebinding/spec) and is meant to simplify the binding of services to an application. Containerized applications that support [Workload Projection](https://github.com/servicebinding/spec#workload-projection) obtain service binding information in the form of volume mounts. The Service Binding Operator reads binding service information and mounts it to the application containers that need it.
...
* For a semi-automatic service binding demonstration, see [How to use Quarkus with the Service Binding Operator](https://developers.redhat.com/articles/2021/12/22/how-use-quarkus-service-binding-operator#create_the_quarkus_application)
* [List of bindable Operators](https://github.com/redhat-developer/service-binding-operator#known-bindable-operators)
...
| `postgresql` | [CrunchyData Postgres](https://operatorhub.io/operator/postgresql)                        | postgres-operator.crunchydata.com/v1beta1 | PostgresCluster
| `mysql`      | [Percona XtraDB Cluster](https://operatorhub.io/operator/percona-xtradb-cluster-operator) | pxc.percona.com/v1-9-0                    | PerconaXtraDBCluster
| `mongo`      | [Percona Mongo](https://operatorhub.io/operator/percona-server-mongodb-operator)          | psmdb.percona.com/v1-9-0                  | PerconaServerMongoDB
|====
...
* For more details about the available properties and how do they work, see the [Workload Projection](https://github.com/servicebinding/spec#workload-projection) part of the Service Binding specification.
