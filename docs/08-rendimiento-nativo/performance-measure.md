...
> **Guia oficial:** <https://quarkus.io/guides/performance-measure>  
> **Fuente:** `docs/src/main/asciidoc/performance-measure.adoc` en [quarkusio/quarkus@3.38.2](https://github.com/quarkusio/quarkus/blob/3.38.2/docs/src/main/asciidoc/performance-measure.adoc)  
...
**📌 NOTE**\
The [spring-quarkus-perf-comparison](https://github.com/quarkusio/spring-quarkus-perf-comparison) repository contains the runnable scripts used for our reference benchmarks, used to produce most of the performance figures on this site.
...
When measuring the footprint of a Quarkus application, we measure [Resident Set Size (RSS)](https://en.wikipedia.org/wiki/Resident_set_size)
...
In order to view the native memory used by the JVM, you can enable the [Native Memory Tracking](https://docs.oracle.com/javase/8/docs/technotes/guides/vm/nmt-8.html) (NMT) feature in hotspot;
...
The [performance lab](https://github.com/quarkusio/spring-quarkus-perf-comparison/blob/main/scripts/perf-lab/main.yml) captures RSS after the first successful request using `pmap`:
...
The [`time-to-1st-request.sh`](https://github.com/quarkusio/spring-quarkus-perf-comparison/blob/main/scripts/perf-lab/scripts/time-to-1st-request.sh) script in the benchmark repository implements this precisely. It records the start time using nanosecond granularity:
...
Throughput is measured using [Hyperfoil](https://hyperfoil.io), a tool designed to avoid the [coordinated omission problem](#coordinated-omission-problem-in-tools).
We run Hyperfoil via [JBang](https://www.jbang.dev), which handles downloading and launching it automatically.

The [performance lab](https://github.com/quarkusio/spring-quarkus-perf-comparison/blob/main/scripts/perf-lab/main.yml) invokes Hyperfoil as follows:
...
The [Hyperfoil configuration](https://github.com/quarkusio/spring-quarkus-perf-comparison/blob/main/scripts/perf-lab/load-tests/load-test-fixed-threads-read-all.hf.yml) defines three phases:
...
A good walkthrough of the issue is [this video](https://www.youtube.com/watch?v=lJ8ydIuPFeU) where Gil Tene the author of wrk2 explains the issue and [Quarkus Insights #22](https://www.youtube.com/watch?v=xdG8b9iDYbE) have John O’Hara from Quarkus performance team show how it can show up.
...
* [Hyperfoil](https://hyperfoil.io) — this is what the [Quarkus benchmark suite](https://github.com/quarkusio/spring-quarkus-perf-comparison) uses
